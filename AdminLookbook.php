...
class AdminLookbook extends AdminTab
{
	...
	/** @var object Lookbook() instance for navigation*/
	private $_Lookbook;
	...
	public function __construct()
	{
		...
		'id_lookbook' => array('title' => $this->l('ID'), 'align' => 'center', 'width' => 30),
		'name' => array('title' => $this->l('Name'), 'width' => 100, 'callback' => 'hideLookbookPosition'),
		...
		$this->_lookbook = AdminLookbookContent::getCurrentLookbook();
		$this->_filter = 'AND `id_parent` = '.(int)($this->_lookbook->id);
	...
	public function display($token = NULL)
	{
		...
		$id_lookbook = (int)(Tools::getValue('id_lookbook', 1));

		$this->getList((int)($cookie->id_lang), !$cookie->__get($this->table.'Orderby') ? 'position' : NULL, !$cookie->__get($this->table.'Orderway') ? 'ASC' : NULL);
		
		echo '<h3>'.(!$this->_listTotal ? ($this->l('There are no sublookbooks')) : ($this->_listTotal.' '.($this->_listTotal > 1 ? $this->l('sublookbooks') : $this->l('sublookbook')))).' '.$this->l('in lookbook').' "'.stripslashes(LookbookObject::hideLookbookPosition($this->_lookbook->getName())).'"</h3>';
		echo '<a href="'.__PS_BASE_URI__.substr($_SERVER['PHP_SELF'], strlen(__PS_BASE_URI__)).'?tab=AdminLookbookContent&add'.$this->table.'&id_parent='.Tools::getValue('id_lookbook').'&token='.($token!=NULL ? $token : $this->token).'"><img src="../img/admin/add.gif" border="0" /> '.$this->l('Add a new sub lookbook').'</a>
	...
	public function postProcess($token = NULL)
	{
		...
		if (Tools::isSubmit('submitAdd'.$this->table))
		{
			...
			{
				if (!LookbookObject::checkBeforeMove($id_lookbook, (int)(Tools::getValue('id_parent'))))
				{
					$this->_errors[] = Tools::displayError('Lookbook cannot be moved here');
					return false;
				}
			}
		}
		/* Change object statuts (active, inactive) */
		elseif (isset($_GET['statuslookbook']) AND Tools::getValue($this->identifier))
		{
			if ($this->tabAccess['edit'] === '1')
			{
				if (Validate::isLoadedObject($object = $this->loadObject()))
				{
					if ($object->toggleStatus())
						Tools::redirectAdmin($currentIndex.'&conf=5'.((int)$object->id_parent ? '&id_lookbook='.(int)$object->id_parent : '').'&token='.Tools::getValue('token'));
					else
		...
		elseif (isset($_GET['position']))
		{
			if ($this->tabAccess['edit'] !== '1')
				$this->_errors[] = Tools::displayError('You do not have permission to edit here.');
			elseif (!Validate::isLoadedObject($object = new Lookbook((int)(Tools::getValue($this->identifier, Tools::getValue('id_lookbook_to_move', 1))))))
				$this->_errors[] = Tools::displayError('An error occurred while updating status for object.').' <b>'.$this->table.'</b> '.Tools::displayError('(cannot load object)');
			elseif (!$object->updatePosition((int)(Tools::getValue('way')), (int)(Tools::getValue('position'))))
				$this->_errors[] = Tools::displayError('Failed to update the position.');
			else
				Tools::redirectAdmin($currentIndex.'&'.$this->table.'Orderby=position&'.$this->table.'Orderway=asc&conf=5'.(($id_category = (int)(Tools::getValue($this->identifier, Tools::getValue('id_lookbook_parent', 1)))) ? ('&'.$this->identifier.'='.$id_category) : '').'&token='.Tools::getAdminTokenLite('AdminLookbookContent'));
		}
		...
		elseif (Tools::getValue('submitDel'.$this->table))
		{
			if ($this->tabAccess['delete'] === '1')
			{
				if (isset($_POST[$this->table.'Box']))
				{
					...
					if ($result)
					{
						$lookbook->cleanPositions((int)(Tools::getValue('id_lookbook')));
	...
	public function displayForm($token=NULL)
	{
		...
		'.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '').'
			<fieldset style="width:520px"><legend><img src="../img/admin/tab-categories.gif" />'.$this->l('Lookbook').'</legend>
		...
		echo '	<p class="clear"></p>
				...
				<label>'.$this->l('Parent lookbook:').' </label>
				<div class="margin-form">
					<select name="id_parent">';
		$categories = LookbookObject::getCategories((int)($cookie->id_lang), false);
		LookbookObject::recurseLookbook($categories, $categories[0][1], 1, $this->getFieldValue($obj, 'id_parent'));
		echo '
					</select>
				</div>
		...
		echo '	<p class="clear"></p>
				...
				<div class="margin-form">
					<input type="submit" value="'.$this->l('Save and back to parent lookbook').'" name="submitAdd'.$this->table.'AndBackToParent" class="button" />
