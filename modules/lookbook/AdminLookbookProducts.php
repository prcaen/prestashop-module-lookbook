...
class AdminLookbookProducts extends AdminTab
{
	...
	public function __construct()
	{
		...
		$this->fieldsDisplay = array(
		  'selected' => array('title' => $this->l('Selected'), 'active' => 'status', 'filter_key' => 'selected', 'align' => 'center', 'type' => 'bool', 'orderby' => false),
	...
	public function postProcess($token = null)
	{
		...
		elseif (isset($_GET['position']))
		{
			...
			if (!$object->updatePosition((int)(Tools::getValue('way')), (int)(Tools::getValue('position'))))
				...
				Tools::redirectAdmin($currentIndex.'&'.$this->table.'Orderby=position&'.$this->table.'Orderway=asc&conf=5'.(($id_category = (!empty($_REQUEST['id_category'])?$_REQUEST['id_category']:'1')) ? ('&id_category='.$id_category) : '').'&token='.Tools::getAdminTokenLite('AdminLookbookContent'));
		}
		else
	...
	public function uploadImageZip($product)
	{
		...
		try
		{
			...
			foreach (scandir($subdir) as $file)
			{
				...
				{
					$image->delete();
					throw new Exception(Tools::displayError('Image format not recognized, allowed <ats are: .gif, .jpg, .png'));
				}
	...
	public function display($token = NULL, $idCms)
	{
    ...
    echo '<input type="hidden" class="id_cms" value="'. $this->_id_cms .'" />';
		...
		$this->displayListFooter(Tools::getAdminTokenLite('AdminLookbookContent'));
	}
	...
	public function displayForm($isMainTab = true)
	{
		...
		if (Tools::getValue('id_category') > 1)
		{
			...
			echo '<br /><br />
			<a href="'.$productIndex.($this->token ? '&token='.Tools::getAdminToken('AdminLookbookContent'.(int)(Tab::getIdFromClassName('AdminCatalog')).(int)($cookie->id_employee)) : '').'">
	...
	function displayFormInformations($obj, $currency)
	{
		...
		if (isset($obj->id))
		{
			...
			if (file_exists(_PS_MODULE_DIR_.'statsproduct/statsproduct.php'))
				echo '&nbsp;-&nbsp;<a href="index.php?tab=AdminStats&module=statsproduct&id_product='.$obj->id.'&token='.Tools::getAdminToken('AdminLookbookContent'.(int)(Tab::getIdFromClassName('AdminStats')).(int)($cookie->id_employee)).'"><img src="../modules/statsproduct/logo.gif" alt="'.$this->l('View product sales').'" title="'.$this->l('View product sales').'" /> '.$this->l('View product sales').'</a>';
		}
		echo '
			<hr class="clear"/>
			<br />
...
<?php
				...
				<a href="?tab=AdminTaxRulesGroup&addtax_rules_group&token='.Tools::getAdminToken('AdminLookbookContent'.(int)(Tab::getIdFromClassName('AdminTaxRulesGroup')).(int)($cookie->id_employee)).'&id_product='.(int)$obj->id.'" onclick="return confirm(\''.$this->l('Are you sure you want to delete entered product information?', __CLASS__, true, false).'\');"><img src="../img/admin/add.gif" alt="'.$this->l('Create').'" title="'.$this->l('Create').'" /> <b>'.$this->l('Create').'</b></a></span>
				';
				if (Tax::excludeTaxeOption())
				{
					echo '<span style="margin-left:10px; color:red;">'.$this->l('Taxes are currently disabled').'</span> (<b><a href="index.php?tab=AdminTaxes&token='.Tools::getAdminToken('AdminLookbookContent'.(int)(Tab::getIdFromClassName('AdminTaxes')).(int)($cookie->id_employee)).'">'.$this->l('Tax options').'</a></b>)';
				...
				if (isset($combArray))
				{
					foreach ($combArray AS $id_product_attribute => $product_attribute)
					{
						...
						<tr'.($irow++ % 2 ? ' class="alt_row"' : '').($product_attribute['default_on'] ? ' style="background-color:#D1EAEF"' : '').'>
							...
							'.(!$product_attribute['default_on'] ? '<a href="'.$currentIndex.'&defaultProductAttribute&id_product_attribute='.$id_product_attribute.'&id_product='.$obj->id.'&'.(Tools::isSubmit('id_category') ? 'id_category='.(int)(Tools::getValue('id_category')).'&' : '&').'token='.Tools::getAdminToken('AdminLookbookContent'.(int)(Tab::getIdFromClassName('AdminCatalog')).(int)($cookie->id_employee)).'">
							...
							<a href="'.$currentIndex.'&deleteProductAttribute&id_product_attribute='.$id_product_attribute.'&id_product='.$obj->id.'&'.(Tools::isSubmit('id_category') ? 'id_category='.(int)(Tools::getValue('id_category')).'&' : '&').'token='.Tools::getAdminToken('AdminLookbookContent'.(int)(Tab::getIdFromClassName('AdminCatalog')).(int)($cookie->id_employee)).'" onclick="return confirm(\''.$this->l('Are you sure?', __CLASS__, true, false).'\');">
							<img src="../img/admin/delete.gif" alt="'.$this->l('Delete this combination').'" /></a></td>
						</tr>';
					}
					echo '<tr><td colspan="7" align="center"><a href="'.$currentIndex.'&deleteAllProductAttributes&id_product='.$obj->id.'&token='.Tools::getAdminToken('AdminLookbookContent'.(int)(Tab::getIdFromClassName('AdminCatalog')).(int)($cookie->id_employee)).'" onclick="return confirm(\''.$this->l('Are you sure?', __CLASS__, true, false).'\');"><img src="../img/admin/delete.gif" alt="'.$this->l('Delete this combination').'" /> '.$this->l('Delete all combinations').'</a></td></tr>';
				}
				else
    ...
    echo '<script type="text/javascript" src="../modules/lookbook/js/lookbook.backend.main.js"></script>';
    echo '<script type="text/javascript" src="../modules/lookbook/js/lookbook.backend.product.js"></script>';
		...
		if ($this->_list)
		{
			...
			foreach ($this->_list AS $tr)
			{
				...
				foreach ($this->fieldsDisplay AS $key => $params)
				{
					...
					if (!isset($params['position']) AND (!isset($this->noLink) OR !$this->noLink) AND !isset($params['active']))
						echo ' onclick="document.location = \'?tab=AdminCatalog&'.$this->identifier.'='.$id.($this->view? '&view' : '&update').$this->table.'&token='.Tools::getAdminTokenLite('AdminLookbookContent').'\'">'.(isset($params['prefix']) ? $params['prefix'] : '');
					else
						echo '>';
	...
	protected function _displayEnableLink($token, $id, $value, $active,  $id_category = NULL, $id_product = NULL)
	{
		...
		echo '<a href="#" class="selected_lookbook_product">
    ...
    echo '
			<a href="?tab=AdminCatalog&'.$this->identifier.'='.$id.'&update'.$this->table.'&token='.Tools::getAdminTokenLite('AdminLookbookContent').'">
