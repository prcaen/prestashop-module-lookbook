...
{	
  ...
  private $_loobook_img_path;
	...
	public function display($token = NULL)
	{
		...
		if($this->_category->id_parent != $this->lookbook_root_id && $this->_category->id_cms_category != $this->lookbook_root_id)
		  echo '<a href="'.$currentIndex.'&id_cms_category='.$id_cms_category.'&add'.$this->table.'&token='.Tools::getAdminTokenLite('AdminLookbookContent').'"><img src="../img/admin/add.gif" border="0" /> '.$this->l('Add a new page').'</a>';
	...
	function postProcess()
	{
              ...
              $look = new LookbookLooks();
                ...
                $this->_errors[] = Tools::displayError('An error occurred while saving the new look');
					...
					if (Tools::isSubmit('submitAddcmsAndPreview'))
					{
						...
						Tools::redirectAdmin($currentIndex.'&id_cms_category='.$cms->id_cms_category.'&conf=3&token='.Tools::getAdminTokenLite('AdminLookbookContent'));
				}
				else
				{
				  // Edit
					  ...
					  if (isset($_FILES['lookbook_img']) AND isset($_FILES['lookbook_img']['tmp_name']) AND !empty($_FILES['lookbook_img']['tmp_name']) AND ($this->levelLookbook == 1 || $this->levelLookbook == 2))
            {
              ...
              $thumb->save($fileName, 'png');
					...
					if (Tools::isSubmit('submitAddcmsAndPreview'))
					{
						...
						Tools::redirectAdmin($currentIndex.'&id_cms_category='.$cms->id_cms_category.'&conf=4&token='.Tools::getAdminTokenLite('AdminLookbookContent'));
				}
			}
		}
		elseif (Tools::getValue('position'))
		{
			...
			elseif (!$object->updatePosition((int)(Tools::getValue('way')), (int)(Tools::getValue('position'))))
				...
				Tools::redirectAdmin($currentIndex.'&'.$this->table.'Orderby=position&'.$this->table.'Orderway=asc&conf=4'.(($id_category = (int)(Tools::getValue('id_cms_category'))) ? ('&id_cms_category='.$id_category) : '').'&token='.Tools::getAdminTokenLite('AdminLookbookContent'));
		}
